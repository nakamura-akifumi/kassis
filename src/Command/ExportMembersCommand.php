...
#[AsCommand(
    name: 'app:export:members',
    description: '利用者（Member）をCSV/XLSXでエクスポートします。',
)]
class ExportMembersCommand extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if ($all) {
            ...
            $defaultPreset = $this->params->has('app.export.members.default_preset')
                ? $this->params->get('app.export.members.default_preset')
                : null;
        ...
        try {
            $tempFile = $this->memberFileService->generateExportFile($members, $format, $columns);
            $defaultName = sprintf('members_%s.%s', date('Y-m-d_H-i-s'), $format);
    ...
    {
        $columns = MemberFileColumns::getExportColumns($this->translator);
    ...
    private function getPresetColumns(string $presetName): ?array
    {
        ...
        if (!$this->params->has('app.export.members.presets')) {
            return null;
        }

        $presets = (array) $this->params->get('app.export.members.presets');
